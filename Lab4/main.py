...
def kanny(path):
    ...
    for x in range (1,(len(gaus)-1)):
        for y in range (1,len(gaus[0])-1):
            Gx=gaus[x+1][y+1]-gaus[x-1][y-1]+gaus[x+1][y-1]-gaus[x-1][y+1]+2*(gaus[x+1][y]-gaus[x-1][y])
            Gy=gaus[x+1][y+1]-gaus[x-1][y-1]+gaus[x-1][y+1]-gaus[x+1][y-1]+2*(gaus[x][y+1]-gaus[x][y-1])
